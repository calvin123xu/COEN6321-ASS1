...
string rotateTile90Degrees(const string& tile) {
	...
	// Rotate 90 degrees clockwise: abcd ¡ú dabc
...
void hybridSurvivorSelection(vector<Individual>& population, int currentGeneration, int maxGenerations) {
	// Parameters for hybrid selection
	double transition_point = 0.5;  // Start transitioning from age to fitness after 50% of generations
	double age_weight = 1.0;        // Full weight on age in the early generations
	double fitness_weight = 0.0;    // No weight on fitness in the early generations
	...
	if (generation_ratio > transition_point) {
		// Gradually shift from age-based to fitness-based
		fitness_weight = (generation_ratio - transition_point) / (1.0 - transition_point);
		age_weight = 1.0 - fitness_weight;
	}

	// Sort population based on a combined score of age and fitness
	std::sort(population.begin(), population.end(), [&](const Individual& a, const Individual& b) {
		double a_score = age_weight * a.age - fitness_weight * a.fitness;  // Higher age and lower fitness is better
		double b_score = age_weight * b.age - fitness_weight * b.fitness;
		return a_score > b_score;
		});

	// Select survivors (top half of the population)
...
int main()
{
	...
	// Seed the random number generator///////////////////////////////////////////////////////////////////////////////ELIMINANTE OTHER SRAND
	std::srand(static_cast<unsigned int>(std::time(0)));
	...
	for (int generation = 0; generation < numberOfGenerations; ++generation) {
		...
		tournamentSelection(parents, populationSize, tournamentSize, parentSize);
		...
		hybridSurvivorSelection(population, generation, numberOfGenerations);
	}
	/*
	for (int i = 0; i < populationSize; ++i) {
	...
	//mainloop, still work on one iteration
	...
	double swapMutationProbability = 0.2;

	//offspring generation
	// edge recombination crossover
	//pick two parents in the pool by ranking selection
	// 
	 // edge combination test
	
	for (int i = 0; i < 50;i++) {
		vector<vector<string>> parent1 = readPuzzle(i);
		vector<vector<string>> parent2 = readPuzzle(i+1);
		...
		cout << randProb << endl;
		if (randProb < swapMutationProbability) {
			// Randomly choose which mutation to apply
			if (rand() % 2 == 0) {
				cout << "Applying swap mutation." << endl;
				swapMutation(offspring);  // Apply swap mutation
			}
			else {
				cout << "Applying rotate mutation." << endl;
