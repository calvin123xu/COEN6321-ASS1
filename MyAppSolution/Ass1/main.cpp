...
int main()
{
	...
	double swapMutationProbability = 0.1;
	...
	for (int generation = 0; generation < numberOfGeneration; ++generation) {
		...
		Individual bestInvididual = generateBestIndividual(population);
		displayPuzzle(bestInvididual.puzzle);
		cout << "the fitness of the best individual is" << bestInvididual.fitness << endl;
	}
